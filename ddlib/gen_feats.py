...
def _get_window_features(
    ...
    try:
        for i in range(1, window + 1):
            pos = str(sentence[span.begin_word_id - i].pos)
            left_pos.append(pos)
    except IndexError:
        pass
    left_pos.reverse()
...
#########################
def get_seq_definition_features(sentence, span1, span2, length_bin_size=5):
    ...
    # pos_seq_feat = "POS_SEQ_[" + " ".join(materialize_span(
    #     sentence, span1, lambda x: str(x.pos))) + "]"
    # yield pos_seq_feat
    ...
    for window_feat in _get_window_features(sentence, temp_span):
        yield window_feat
    for same_token_feature in _get_same_token_definition_feature(sentence,span1,span2):
        yield same_token_feature
    if is_upper(sentence[span2.begin_word_id].word):
        yield "EXPLAIN_STARTS_WITH_CAPITAL"
    else :
        yield "EXPLAIN_NOT_STARTS_WITH_CAPITAL"
    if is_candidate(sentence,span1,span2) :
        yield "IS_CANDIDATE"
    else :
        yield "NOT_CANDIDATE"
